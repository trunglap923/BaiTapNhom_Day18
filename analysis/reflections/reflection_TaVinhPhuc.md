...
- Các hàm/class chính đã viết: 
  - `CrossEncoderReranker`: Load mô hình `BAAI/bge-reranker-v2-m3` và thực hiện hàm `rerank()` để tính predict scores giữa cặp query-doc, sau đó sắp xếp lấy ra top-k `RerankResult`.
...
- Kết nối với bài giảng (slide nào): Kết nối trực tiếp với **Slide 22 / 42 ("Reranking — Highest ROI Optimization")** — Bài giảng chỉ rõ chiến lược RAG chuẩn là Retrieve top-20 bằng Dense Search/BM25 (Bi-encoder, siêu nhanh ~1ms) rồi Rerank lại lấy top-3 bằng CrossEncoder (chậm hơn ~50ms nhưng đổi lấy +15-25% precision). Đặc biệt slide có nhấn mạnh tiếng Việt thì dùng model `bge-reranker-v2-m3` là tối ưu nhất.
...
- Module nào muốn thử tiếp: Mình rất tò mò về M5 (Enrichment Pipeline), vì kỹ thuật dùng LLM để chèn thêm ngữ cảnh vào văn bản trước khi embed (Contextual Prepend) hay sinh câu hỏi ảo (HyQA) nghe rất hứa hẹn để giải quyết bài toán lệch từ vựng từ gốc rễ.
